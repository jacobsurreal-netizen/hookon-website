...
export function FeaturesSection() {
  ...
  return (
    <section id="services" className="relative py-38 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="max-w-8x1 mx-auto">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-8xl font-bold text-slate-50/70 mb-8 text-balance">{t.features.title}</h2>
          <p className="text-lg font-bold text-orange-600/80 max-w-3´2xl mx-auto">{t.features.subtitle}</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {features.map((feature, idx) => (
            <div
              key={idx}
              className="group relative p-4 rounded-3xl backdrop-blur-xl bg-gradient-to-br from-black/30 via-white/5 to-black/10 border border-white/30 hover:border-orange-200/60 transition-all duration-300 hover:shadow-xl hover:shadow-orange-500/10"
            >
    ...
    group-hover:rotate-5">
              ...
              <p className="text-white font-bold leading-relaxed">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
