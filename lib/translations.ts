export const translations = {
  en: {
    nav: {
      about: "About",
      services: "Services",
      work: "Work",
      contact: "Get Started",
    },
    hero: {
      title: "Hook Your Audience with AI-Powered Advertising",
      subtitle: "Engage your audience with AI-powered advertising",
      cta1: "Get Started",
      cta2: "See Our Work",
    },
    features: {
      title: "Unlock AI-Driven Growth",
      subtitle: "Advanced capabilities designed for modern marketing teams",
      feature1: {
        name: "AI-Powered Advertising",
        description:
          "Drive engagement with smart, targeted AI-driven ads that reach the right audience at the right time",
      },
      feature2: {
        name: "Brand Strategy",
        description: "Build a strong brand identity with innovative AI techniques and strategic positioning",
      },
      feature3: {
        name: "Creative Solutions",
        description: "Boost your brand's creativity and reach with AI-enhanced campaigns and content creation",
      },
    },
    benefits: {
      title: "Why Choose hookon",
      subtitle: "Proven results for businesses of all sizes",
      benefit1: {
        name: "Performance",
        description: "Data-driven campaigns that deliver measurable ROI and scalable growth",
      },
      benefit2: {
        name: "Speed",
        description: "Launch campaigns in days, not weeks, with AI-accelerated workflows",
      },
      benefit3: {
        name: "Scalability",
        description: "Grow from startup to enterprise without changing your workflow",
      },
    },
    caseStudies: {
      title: "Our Work",
      subtitle: "Real results from real clients",
      project: "Project",
      comingSoon: "Coming Soon",
    },
    cta: {
      title: "Ready to Transform Your Marketing?",
      subtitle: "Join forward-thinking brands using AI for competitive advantage",
      button: "Book a Demo",
    },
    contact: {
      title: "Get in Touch",
      subtitle: "Let's discuss how we can transform your marketing with AI",
      namePlaceholder: "Your Name",
      emailPlaceholder: "your@email.com",
      messagePlaceholder: "Type your message…",
      button: "Send Message",
      sending: "Sending...",
      successMessage: "Thank you! We'll get back to you soon.",
    },
    footer: {
      tagline: "AI-Powered Advertising Studio",
      copyright: "© 2026 hookon. All rights reserved.",
      contact: "Contact",
      email: "hello@hookon.io",
    },
  },
  cs: {
    nav: {
      about: "O nás",
      services: "Služby",
      work: "Naše práce",
      contact: "Začít",
    },
    hero: {
      title: "Posuňte svou značku pomocí AI-řízených reklam",
      subtitle: "Zaujměte své publikum pomocí reklamy s využitím umělé inteligence",
      cta1: "Začít",
      cta2: "Podívejte se na naši práci",
    },
    features: {
      title: "Odemkněte AI-řízený růst",
      subtitle: "Pokročilé možnosti určené pro moderní marketingové týmy",
      feature1: {
        name: "AI-řízená reklama",
        description:
          "Zvyšte engagement s inteligentnímí, cílenými AI-řízenými reklamami, které dosáhnou správné cílové skupiny",
      },
      feature2: {
        name: "Strategie značky",
        description: "Vytvořte silnou identitu značky pomocí inovativních AI technik a strategického umístění",
      },
      feature3: {
        name: "Kreativní řešení",
        description: "Zvyšte kreativitu a dosah své značky pomocí AI-vylepšených kampaní a tvorby obsahu",
      },
    },
    benefits: {
      title: "Proč zvolit hookon",
      subtitle: "Ověřené výsledky pro firmy všech velikostí",
      benefit1: {
        name: "Výkon",
        description: "Datově řízené kampaně, které přinášejí měřitelné ROI a škálovatelný růst",
      },
      benefit2: {
        name: "Rychlost",
        description: "Spusťte kampaně za dny, ne týdny, pomocí AI-zrychleného pracovního postupu",
      },
      benefit3: {
        name: "Škálovatelnost",
        description: "Růst od startupu k podniku bez změny vašeho pracovního postupu",
      },
    },
    caseStudies: {
      title: "Naše práce",
      subtitle: "Skutečné výsledky od skutečných klientů",
      project: "Projekt",
      comingSoon: "Již brzy",
    },
    cta: {
      title: "Jste připraveni transformovat svůj marketing?",
      subtitle: "Připojte se k forward-thinking značkám, které používají AI pro konkurenční výhodu",
      button: "Naplánovat demo",
    },
    contact: {
      title: "Kontaktujte nás",
      subtitle: "Pojďme si promluvit o tom, jak můžeme transformovat váš marketing pomocí AI",
      namePlaceholder: "Vaše jméno",
      emailPlaceholder: "váš@email.com",
      messagePlaceholder: "Napište svou zprávu…",
      button: "Odeslat zprávu",
      sending: "Odesílání...",
      successMessage: "Děkujeme! Brzy se vám ozveme.",
    },
    footer: {
      tagline: "  ____ h o o k o n ____",
      copyright: "© 2026 hookon. Všechna práva vyhrazena.",
      contact: "Kontakt",
      email: "hello@hookon.io",
    },
  },
}
